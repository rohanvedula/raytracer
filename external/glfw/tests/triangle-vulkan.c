...
static void demo_prepare_buffers(struct demo *demo) {
    ...
    if (surfCapabilities.currentExtent.width == 0xFFFFFFFF) {
        ...
        // of the imagesa1 requested, which must fit within the minimum and
        // maximum values.
    ...
    // If maxImageCount is 0, we can ask for as many imagesa1 as we want;
    // otherwise we're limited to maxImageCount
    if ((surfCapabilities.maxImageCount > 0) &&
        (desiredNumOfSwapchainImages > surfCapabilities.maxImageCount)) {
        // Application must settle for fewer imagesa1 than desired:
        desiredNumOfSwapchainImages = surfCapabilities.maxImageCount;
    }
    ...
    // presentable imagesa1 once the platform is done with them.
